...
extern uint16      ad_once   (ADCn, ADC_Ch, ADC_nbit);    //采集一次一路模拟量的AD值

extern uint16      ad_mid    (ADCn, ADC_Ch, ADC_nbit);    //中值滤波后的结果
extern uint16      ad_mid_last (ADCn, ADC_Ch, ADC_nbit);
extern float      ad_ave    (ADCn, ADC_Ch, ADC_nbit, uint8 N); //均值滤波后的结果
extern float      ad_ave_mid(ADCn , ADC_Ch , ADC_nbit , uint8 N);

extern uint16      ad_flt    (ADCn, ADC_Ch, ADC_nbit);    //与前几次采样均值滤波   速度快于ad_ave很多，如果相邻采样时间比较短，则采样精确于 ad_once。即采样间距较短的情况下优先考虑使用这个
